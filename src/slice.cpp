...
al::Vec3f find_slicing_dir(const std::vector<al::Vec3f> &v, size_t num_samples, float sample_radius, size_t MAX_NEIGHBORS)
{
  ...
  for (size_t i = 0; i < num_samples; i += 1) {
    ...
    if (neighbors.size() == 0) { return averaged_averaged; }
    // sort the neighbors
...
std::vector<Vec3f> rotate_to_align_to_z(const std::vector<Vec3f> &data, Vec3d layer_dir, Vec3f center_of_rotation)
{
  auto v = data; // copy
  for (auto &p : v) {
    al::Mat4f t;
    ...
    t *= rot_mat;
    t *= al::Matrix4f::translation(-center_of_rotation); // to center of rotation
