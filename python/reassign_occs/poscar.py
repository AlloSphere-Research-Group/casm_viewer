...
class Site:
    ...
    def write(self, file):
        ...
        file.write(asbytes(self.SD_FLAG))
        file.write(asbytes(self.occupant))
        file.write(asbytes('\n'))
...
class Poscar:
    ...
    def write(self, filename, sort=True):
        ...
        file.write(asbytes(str(self.header)+'\n'))
        file.write(asbytes(str(self.scaling)+'\n'))
        np.savetxt(file,self._lattice.astype(np.float), newline = '\n', fmt='%.8f' )
        ...
        if sort:
            ...
            file.write(asbytes(type_line+'\n'))
            file.write(asbytes(num_line+'\n'))
            file.write(asbytes(self.coord_mode+'\n'))
            for atom in sorted(d.keys()):
                for tb in d[atom]:
                    tb.write(file)
        else:
            file.write(asbytes(' '.join(self.type_atoms_alias) + '\n'))
            file.write(asbytes(' '.join([str(x) for x in self.num_atoms]) + '\n'))
            if(self.SD_FLAG):
                file.write(asbytes('Selective Dynamics\n'))
