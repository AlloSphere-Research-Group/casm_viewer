...
public:
    ...
    // but perhaps we should have a higher level class that solves this issue for the general case
    ...
    // Modification of parameter space. These are not protected by the lock, the user needs to lock as needed
    void sort(std::function<bool(const std::pair<std::string, float> & a, const std::pair<std::string, float> & b)> sortFunction =
