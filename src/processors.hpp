...
public:
    ...
    void setParams(std::string id, std::string condition, std::string datasetId, std::string timeStep = "") {
        mId = id;
        mCond = condition;
        ...
        std::string template_pos_path = File::conformPathToOS(root_path + mDatasetId + "/cached_output/template_POSCAR");

        // Try to find "template_POSCAR"
        if (!File::exists(template_pos_path)) {
            std::cout << "Failed to find template file. Call it 'template_POSCAR'." << std::endl;
        }
        setOutputFileNames({sanitizeName(template_pos_path + mDatasetId + "_" + folder + "_" + mCond + "_" + mTimeStep)});
    }
...
//    }
    ...
    std::map<std::string, std::string> configuration() {
        ...
        std::string folder = File::conformDirectory(mId);
        std::string subdir = File::conformPathToOS(folder + "conditions." + mCond + "/");
...
//        config["root_path"] = File::conformDirectory(root_path);
        ...
        std::string final_state_path = subdir + "final_state.json";    // final_state.json file
        config["final_state_path"] = File::conformPathToOS(root_path + mDatasetId + "/" + final_state_path);

        config["template_path"] = File::conformPathToOS(root_path + mDatasetId + "/cached_output/template_POSCAR");
...
public:
    ...
    std::map<std::string, std::string> configuration() {
        ...
        config["__output_name"] = dataset + "_" + temp_interest + "_" + sanitizeName(subdir) + "_" + yLabelSanitized + "_graph.png";
        appendFlag(config["__output_name"], FLAG_OUTPUT_NAME);

        config["__output_path"] = outputDirectory();

        if (multi) {
//            appendFlag(pythonScriptsPath + "/graphing/plot_multi.py", FLAG_SCRIPT);
//            appendFlag(casm_path);
//            appendFlag(subpath1);
//            appendFlag(subpath2);
...
//            appendFlag(title + "_" + temp_interest + "_graph-multi.png", FLAG_OUTPUT_NAME);
        } else {
